...
def get_video_feature_extractor(
    ...
    if vid_base_arch == 'r2plus1d_18':
        model = r2plus1d_18(pretrained=pretrained, larger_last=larger_last)
        if not pretrained:
...
class GDT(nn.Module):
    def __init__(
        ...
        self.video_network = VideoBaseNetwork(
            vid_base_arch, 
            pretrained=pretrained, larger_last=larger_last,
        )
...
class Stica_Transformer(nn.Module):
    def __init__(
        self,
        vid_base_arch='r2plus1d_18', 
        aud_base_arch='resnet9',
        ...
        super(Stica_Transformer, self).__init__()
        print('Using StiCa-Transformer model')
        ...
        self.num_sec = args.num_sec if args else 1
        ...
        # Aggregation module
        self.video_pooling = TransformerPooling(
            emb_dim=encoder_dim, 
        ...
        if use_mlp:
            print("Using MLP projection layer")
            self.mlp_v = MLP(encoder_dim, num_classes, n_hidden=n_hidden)
            self.mlp_a = MLP(encoder_dim_a, num_classes)
        else:
            print("Using Linear Layer")
            self.mlp_v = nn.Linear(encoder_dim, num_classes)
    ...
    def forward(self, img, spec, return_after_base=False):
        ## B: batch size
        ...
        # B V H => B H
        img_features = self.video_pooling(img_features)
...
def load_model(
    model_type='stica_fmcrop_transformer',
    vid_base_arch='r2plus1d_18', 
    aud_base_arch='resnet9',
    pretrained=False, 
    norm_feat=True, 
    ...
    # Cross-modal GDT
    if model_type == 'stica_fmcrop_transformer':
    ...
    elif model_type == 'stica_fmcrop':
        print('Using Stica-Transformer')
        model = Stica_Transformer(
            vid_base_arch=vid_base_arch, 
            aud_base_arch=aud_base_arch,
            pretrained=pretrained, 
            norm_feat=norm_feat, 
