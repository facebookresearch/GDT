...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
                 ...
                 zero_init_residual=False, larger_last=False):
        ...
        self.layer4 = self._make_layer(block, conv_makers[3], 512 if not larger_last else 640, layers[3], stride=2)

        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.fc = nn.Linear((512 if not larger_last else 640) * block.expansion, num_classes)
...
def r2plus1d_34(pretrained=False, progress=True, **kwargs):
...
if __name__ == '__main__':
    ...
    kwargs = {'larger_last':True}
    model = r2plus1d_18(pretrained=False, progress=True, **kwargs)
    print(sum(p.numel() for p in model.parameters() if p.requires_grad)/ 1e6)