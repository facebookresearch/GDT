...
def random_resize_crop_video(
    ...
    i, j, h, w = get_params(clip, scale, ratio)
    return resized_crop(clip, i, j, h, w, size, interpolation_mode)
...
def gaussian_blur(images):
    ...
    Args:
        images (tensor): images to perform color normalization. Dimension is
                `num frames` x `channel` x `height` x `width`.
    Returns:
        out_images (tensor): the blurred images, the dimension is
            `num frames` x `channel` x `height` x `width`.
    """
    ...
    # Input: (B, C, H, W)
    # Output: (B, C, H, W)
    # GaussianBlur(kernel_size, sigma, border_type='reflect')

    gauss = kornia.filters.GaussianBlur2d((11, 11), (.1, 2.))
    return gauss(images)
...
def clip_augmentation(
    ...
    if use_random_resize_crop:
        frames = random_resize_crop_video(frames, crop_size, scale=(min_scale, max_scale))
        frames, _ = horizontal_flip(0.5, frames)
    else:
    ...
    # print(frames.mean(), frames.min(), frames.max())
